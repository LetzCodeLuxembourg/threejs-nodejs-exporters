...
class USDZExporter {
  ...
  async parse(scene, unitsPerMeter) {

    let output = buildHeader(unitsPerMeter);
    ...
    scene.traverse((object) => {
      ...
      if (object.isMesh) {
        ...
        if (material.map) textures[material.map.uuid] = material.map;
        if (material.normalMap) textures[material.normalMap.uuid] = material.normalMap;
        if (material.aoMap) textures[material.aoMap.uuid] = material.aoMap;
        if (material.roughnessMap) textures[material.roughnessMap.uuid] = material.roughnessMap;
        if (material.metalnessMap) textures[material.metalnessMap.uuid] = material.metalnessMap;
        if (material.emissiveMap) textures[material.emissiveMap.uuid] = material.emissiveMap;
...
function buildHeader(unitsPerMeter = 0.1) {
...
function buildMeshVertexCount(geometry) {
  ...
  const count = geometry.index ? geometry.index.array.length : geometry.attributes.position.count;
...
function buildMaterial(material) {
  ...
  if (material.roughnessMap) {
    ...
    parameters.push(`${pad}float inputs:roughness = ${material.roughness || 0}`);
  ...
  if (material.metalnessMap) {
    ...
    parameters.push(`${pad}float inputs:metallic = ${material.metalness || 0}`);
